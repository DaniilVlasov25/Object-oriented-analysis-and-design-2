...
// SWORD PHYSICAL
class SwordPhysical : public Weapon {
private:
    int sharpness;

public:
    SwordPhysical(int sharp)
        : Weapon("Sword", 100, 50), sharpness(sharp) {
    }
...
// SWORD MAGIC
class SwordMagic : public Weapon {
private:
    int sharpness;

public:
    SwordMagic(int sharp)
        : Weapon("Sword", 100, 50), sharpness(sharp) {
    }

    int attack() override {
        int totalPower = basePower + sharpness;
        return totalPower * 3 + 10;
    }
...
// BOW PHYSICAL
class BowPhysical : public Weapon {
private:
    int range;

public:
    BowPhysical(int rng)
        : Weapon("Bow", 80, 30), range(rng) {
    }
...
// STAFF PHYSICAL
class StaffPhysical : public Weapon {
private:
    int manaCost;

public:
    StaffPhysical(int mana)
        : Weapon("Staff", 60, 40), manaCost(mana) {
    }
...
// STAFF MAGIC
class StaffMagic : public Weapon {
private:
    int manaCost;
...
void CreateWeapon() {
    ...
    if (selectedWeaponType == 0) {
        if (selectedDamageType == 0)
            weapon = new SwordPhysical(10);
        else if (selectedDamageType == 1)
            weapon = new SwordMagic(10);
        else if (selectedDamageType == 2)
            weapon = new SwordFire(10);
    }
    else if (selectedWeaponType == 1) {
        if (selectedDamageType == 0)
            weapon = new BowPhysical(15);
        else if (selectedDamageType == 1)
            weapon = new BowMagic(15);
        else if (selectedDamageType == 2)
            weapon = new BowFire(15);
    }
    else if (selectedWeaponType == 2) {
        if (selectedDamageType == 0)
            weapon = new StaffPhysical(20);
        else if (selectedDamageType == 1)
            weapon = new StaffMagic(20);
        else if (selectedDamageType == 2)
            weapon = new StaffFire(20);
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        CreateWindowW(L"STATIC", L"Тип оружия:",
            WS_VISIBLE | WS_CHILD, 20, 55, 100, 25, hWnd, nullptr, hInst, nullptr);
        ...
        CreateWindowW(L"STATIC", L"Тип урона:",
            WS_VISIBLE | WS_CHILD, 20, 115, 100, 25, hWnd, nullptr, hInst, nullptr);
